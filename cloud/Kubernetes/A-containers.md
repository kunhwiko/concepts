...
##### Containers
```
Containers represent instances of images and run as processes on a host operating system.
These processes are executed in an isolated Linux namespace and controlled by cgroups.
These processes share the host's kernel by making syscall requests.
```

##### Containers vs Virtual Machines
```
a) VMs are packed with their own OS and virtualize an entire machine down to the hardware layers.
   Containers share the host OS as processes in isolated user spaces.
b) VMs are relatively slow to start up as they need to boot up an OS.
   Containers are lightweight as they share the kernel of host machines while still providing environmental consistency.
