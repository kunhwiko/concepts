...
"Capacity Reservation", which reserves on-demand capacity in a specific zone for any duration.
```
...
instance, the response for that request is allowed to reach the instance regardless of inbound security group rules. 
Similarly, responses to allowed inbound traffic are allowed to leave the instance regardless of outbound rules.
```
