...
a) CoreDNS is a pod that functions as a DNS server in the cluster. It has become the default over KubeDNS.
     ...
     * Every pod receives an A record: <pod-ip-address>.<namespace>.pod.cluster.local. Pods by default do not have an
       A record for the pod name. Refer to the following: https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/
