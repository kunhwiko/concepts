...
a) CoreDNS is a pod that functions as a DNS server in the cluster. It has become the default over KubeDNS.
     * Every service receives an A record for <service-name>.<namespace>.svc.cluster.local
     * Every pod receives an A record for <pod-ip-address>.<namespace>.pod.cluster.local. Note that by default, pods do 
       not have an A record for the pod name. 
     * Refer to the following: https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/
b) When a pod is created, kubelets will write the IP address of the DNS server to /etc/resolv.conf on the pod. 
```
